...
\item{Start on annotated bibliography and continue to look at systems/logistics}
\item{Brainstorm other research ideas}
\item{Work on Endodemog model with year effects and visualizing the model for Stan}
\item{Work on Endodemog data cleaning script}
\end{itemize}
\end{document}